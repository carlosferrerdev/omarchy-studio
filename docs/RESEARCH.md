...
- [pw-top](https://docs.pipewire.org/page_man_pw-top_1.html): driver quantum/rate and error statistics are observable, but followers have different semantics. The following increment implements bounded driver sampling; XRUN counters remain deferred. Unknown values remain null.
...
Systemd service state and a successful PipeWire snapshot are separate observations. Missing tools, failed probes, malformed data and empty hardware inventories remain distinguishable. No readiness claim certifies low-latency or live-performance suitability. Deeper realtime observations, UMP details, routing validation, hardware capabilities and journal redaction require further work. Profiles, privilege escalation and rollback are future design work; no configuration commands ship in this milestone.

## Driver sampling follow-up

Rechecked the installed PipeWire 1.6.8 and its [pw-top source](https://github.com/PipeWire/pipewire/blob/1.6.8/src/tools/pw-top.c) before implementing the sampler. Batch output has a fixed 16-character FORMAT field and explicit ` + ` / ` = ` follower prefixes. The first view can contain unpopulated creating nodes. Two iterations normally provide an initialized view after the one-second timer; only positive clocks on running drivers are accepted. Profiler absence can terminate without a reliable nonzero exit status, so exit success alone is insufficient. Names can be truncated to 128 bytes; mismatches remain unavailable rather than guessing a device.

[pw-profiler](https://docs.pipewire.org/page_man_pw-profiler_1.html) uses the same profiler interface, but its normal file/chart workflow is unnecessary for a small on-demand diagnostic. We keep Bash/jq and the existing bounded probe boundary. The ERR column combines XRUNs and errors, so no pure XRUN count is exported. Synthetic active/multiple-driver fixtures and passive live idle observations validate the initial sampler; sustained active hardware validation remains opt-in.
